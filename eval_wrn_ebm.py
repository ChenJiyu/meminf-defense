...
def test_clf_dir(args):
    if args.print_to_log:
        sys.stdout = open(f'{args.model_dir}/eval_clf_log.txt', 'w')

    c_dict = {}
    device = t.device('cuda' if t.cuda.is_available() else 'cpu')
    for model_name in tqdm(os.listdir(args.model_dir)):
        model_path = os.path.join(args.model_dir, model_name, 'best_valid_ckpt.pt')
        if not os.path.exists(model_path):
            continue
        f = load_model(args, model_path)
        print('model:', model_path)
        loss, correct = test_clf(f, args, device)
        c_dict[model_name] = correct
    
    model_prefix_set = set(map(lambda x : '_'.join(x.split('_')[:-1]), c_dict.keys()))
    temp = []
    for model_prefix in model_prefix_set:
        for k in c_dict.keys():
            if model_prefix in k:
                temp.append(c_dict[k])
        print(model_prefix, sum(temp)/len(temp))
        temp = []
